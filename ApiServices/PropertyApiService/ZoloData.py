...
def PropertyData():
    ...
    city_ids = [
        city.get("city_name")
        for city in collection.find({}, {"city_name": 1, "_id": 0})
    ]
            ...
            for property in properties:
                ...
                property_data = {
                    ...
                    "Location_coordinates": {
                        "latitude": (
                            basic_data.get("location", [])[0]
                            if len(basic_data.get("location", [])) > 0
                            else None
                        ),
                        "longitude": (
                            basic_data.get("location", [])[1]
                            if len(basic_data.get("location", [])) > 1
                            else None
                        ),
                    },
                ...
                existing_property = collection1.find_one(
                ...
                if existing_property:
                    ...
                    print(
                        f"Property {property_data['property_id']} updated in MongoDB."
                    )
                else:
                    collection1.insert_one(property_data)
                    print(
                        f"Property {property_data['property_id']} inserted into MongoDB."
                    )
