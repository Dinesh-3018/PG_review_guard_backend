...
def run(playwright):
    ...
    url = (
        "https://vikaspedia.in/education/current-affairs/states-and-districts-of-india"
    )
    page.goto(url)
    ...
    for header in district_headers:
        ...
        State_Iso2 = state_collection.find_one(
            {"name": {"$regex": district_name, "$options": "i"}}, {"iso2": 1, "_id": 0}
        )
            ...
            for row in table_body.find_all("tr"):
                ...
                for district in districts:
                    ...
                    if district_name not in states_and_uts:
                        ...
                        exits = Districts_collection.find_one(
                            ...
                            Districts_collection.insert_one(
                                {
                                    ...
                                    "districts_name": district_name,
                                    "state_iso": (
                                        State_Iso2.get("iso2") if State_Iso2 else None
                                    ),
                                    "state_name": (
                                        State_Name.get("name") if State_Iso2 else None
                                    ),
                                }
                            )
